...
def profile(request):
    if request.method == 'POST':
        ...
        if u_form.is_valid() and p_form.is_valid():
            ...
            if (nowy_email != old_email and User.objects.filter(
                    email=nowy_email).exists()) or '@jbiernacki.pl' in nowy_email:
...
def contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                send_mail(f"User: {request.user.username} Tytuł: " + subject, message, from_email,
                          [getattr(settings, "EMAIL_HOST_USER")])
                messages.success(request, "Wiadomość została wysłana")
                if form.cleaned_data['checkbox']:
                    send_mail("Tytuł: " + subject,
                              'To jest wysłana przez ciebie wiadomość przez strone https://galeria.jbiernacki.pl/ : '
                              '\n\n' + message,
                              'Galeria', [from_email])
                return redirect('index')
            except:
